...
class SemanticSpace:
    ...
    def create_query_vector(self, q):
        ...
        for t in q:
            for index, pair in enumerate(self.I.terms):
                if pair[0] == t:
                    ret_q[index] = 1.0
    ...
    def fold_in_query(self, q):
        # new query is qTS^{-1}
        no_terms = self.T.shape[0]  # rows
        rank = self.T.shape[1]  # columns

        # Create vector for folded in query
        fol_q = [0.0 for i in range(rank)]
        q_head = [0.0 for i in range(rank)]
        ...
        for i in range(rank):
            for j in range(no_terms):
                fol_q[i] += q[j] * self.T[j][i]

        for i in range(rank):
            q_head[i] += fol_q[i] * self.S_inv[i]
    ...
    def cosine_with_doc(self, q, doc):
        ...
        m_q = np.linalg.norm(self.Dt[:, doc][0:self.max_dimension] * self.S[0:self.max_dimension])
        m_d = np.linalg.norm(q[0:self.max_dimension] * self.S[0:self.max_dimension])
        ...
        for i in range(self.max_dimension):
            calc += q[i] * self.S_sq[i] * self.Dt[i, doc]

        cos_with_doc = calc / (m_q * m_d)
        return cos_with_doc
    ...
    def cosine_with_term(self, t1, t2):
        ...
        m_t1 = np.linalg.norm(self.T[t1, :][0:self.max_dimension] * self.S[0:self.max_dimension])
        m_t2 = np.linalg.norm(self.T[t2, :][0:self.max_dimension] * self.S[0:self.max_dimension])
        ...
        # TODO:  Complete this method
        for i in range(self.max_dimension):
            calc += self.T[t1, i] * self.S_sq[i] * self.T[t2, i]

        cos_with_terms = calc / (m_t1 * m_t2)

        return cos_with_terms